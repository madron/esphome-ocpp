...
void ChargePoint::send_message_(QueuedMessage message) {
...
bool ChargePoint::pop_queued_message(std::string *message, uint32_t now_millis) {
    ...
    if (this->debug_ocpp_messages_)
        ESP_LOGD(TAG, "%s >> %s", this->connection_id_.c_str(), queued_message.payload.c_str());
...
void ChargePoint::handle_ocpp_call_(const OcppMessage &call) {
    ...
    if (call.action == "BootNotification") {
        ...
        this->send_message_({this->protocol_.make_boot_notification_response(call.unique_id), OcppMessageType::CALL_RESULT,
                             call.unique_id});
